"""Typer ベースの CLI アプリケーションを組み立てるファクトリ。"""
...
def _build_action_command(
    ...
    def command(params_json: str) -> None:
        # command関数は、カスタムコマンドの関数をラップしている
        # ヘルプ文は関数定義後に action.description を __doc__ に代入する。
        try:
            parsed = input_schema.model_validate_json(params_json)
            result = controller._run_sync(func(parsed))  # カスタムコマンドの関数を実行
        except ValidationError as exc:
            typer.secho(str(exc), fg=typer.colors.RED, err=True)
            raise typer.Exit(code=2) from exc
        except StationError as exc:
