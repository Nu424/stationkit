...
別プロジェクトから git 経由で依存追加する場合:

```bash
uv add "stationkit @ git+https://github.com/Nu424/stationkit.git"
```

この方法でも `stationkit` 本体に加えて、Sequence Web App の server wrapper
（`apps.sequence_app.server.create_sequence_app_server`）を import できます。
React frontend の build 済み静的ファイルも同梱されるため、`frontend_dist_dir`
を指定しなくても Sequence Web App を配信できます。
...
app = create_sequence_app_server(MyDeviceController())
